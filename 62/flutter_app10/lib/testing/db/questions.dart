...
class Questions {
  ...
  static Future<List<Question>> _parseQuestions() async {
    String data = await rootBundle.loadString("assets/texts/question2.txt");
...
//        wrong3: lines[i + 4],
  ...
  static Future<FutureOr<void>> _onCreate(Database db, int version) async {
    await db.execute(
        "CREATE TABLE questions(id INTEGER PRIMARY KEY, question TEXT, answer TEXT, wrong1 TEXT, wrong2 TEXT, completed BOOLEAN);");
    ...
    await db.transaction((txn) async {
      for (Question question in questions) {
        await txn.rawInsert("INSERT INTO questions("
            "question, answer, wrong1, wrong2, completed) VALUES("
            "\"${question.question}\","
            "\"${question.answer}\","
            "\"${question.wrong1}\","
            "\"${question.wrong2}\","
